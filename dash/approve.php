...
<html>
  ...
  <?php
    ...
    if (mysqli_num_rows($sql) > 0) {
        echo '<h1 class="w3-xlarge"> Recent submissions </h1>
  ...
  <?php
    ...
    if (isset($_POST['id'])) {
        ...
        if ($sql) {
          ...
          if ($rate >= 0) {
            $stmt = "INSERT INTO liveArt (id, title, src, tags, rate, artist, data) VALUES (NULL, '$name', '$source', '$tags', '$rate', '$artist', '$data')";
            $sql = mysqli_query($db, $stmt);
            if ($sql) {
